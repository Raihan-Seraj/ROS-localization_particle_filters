...
class robot(): 
	...
	self.baseMapReady = False
	while(self.baseMapReady == False): 
	    #wait for baseMapReady
            pass
	...
	for move in self.moves: 
	    ...
	    move_function(angle,0)
	        ...
	        self.turn_all_particles(particles,angle, 0.06)	 
	    else: 
		...
		if(self.move_index == 0): 
		    self.move_all_particles(particles,distance, 1.6,1.6) 
		...
		for particle in particles: 
		    ...
		    ptot = sum(pz)
		    #ptot = np.product(pz)
		    #if(goodSenseCount > 30): 
			#print "I found the one" 
			#ptot = np.product(pz)
		    #if(badSenseCount > 20 ) : 
			#print "badSenseCount = ",badSenseCount 
		    if(badParticle or badSenseCount >20): 
			particle.weight = 0
		    else: 
		        #print "ptot = ",ptot
			#print "old weight = ", particle.weight
			#print "new weight = ",ptot
			#print "new weight = ", (1/(1+m.exp(-ptot)))
			particle.weight = particle.weight*(1/(1+m.exp(-ptot)))
			#particle.weight = particle.weight*ptot
		...
		for i in range(self.config['num_particles']): 
		    ...
		    tempX = tempParticle[0].x + r.gauss(0,self.config['resample_sigma_x'])
		    #print "Temp X = ",tempX
		    if tempParticle[0].index in badParticles: 
			print "It picked a bad Particle!!!"
		    tempY = tempParticle[0].y + r.gauss(0,self.config['resample_sigma_y'])
		    #print "Temp Y = ",tempY
		    tempAngle = tempParticle[0].angle + r.gauss(0,self.config['resample_sigma_angle'])
...
#	self.particlecloud_publisher.publish(self.pose_array)
	...
	self.baseMapReady = True
	print self.baseMapReady
	"""
	particles = []
        self.pose_array = PoseArray()
        self.pose_array.header.stamp = rospy.Time.now()
	...
	for i in range(self.config['num_particles']):
	    #randomX = r.randint(0,self.baseMap.width)
	    #randomY = r.randint(0,self.baseMap.height)
	    randomX = message.info.width*np.random.rand()
	    randomY = message.info.height * np.random.rand()
	    '''
	    while(self.baseMap.get_cell(randomX,randomY) != 0): 		
                randomX = message.info.width*np.random.rand()
	        randomY = message.info.height * np.random.rand()
	...
	for move in self.moves: 
		...
		for particle in particles: 
		    ...
		    if(badSenseCount > 20 ) : 
			print "badSenseCount = ",badSenseCount 
		    if(badParticle or badSenseCount >55): 
			particle.weight = 0
		    else: 
		        #print "ptot = ",ptot
			particle.weight = particle.weight*(1/(1+m.exp(-ptot)))
